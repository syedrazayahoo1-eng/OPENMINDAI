...
public interface IAIProvider
{
    ...
    /// <summary>
    /// Streams a completion for the given request as it is generated, yielding
    /// incremental text chunks (tokens) rather than waiting for the full response.
    /// </summary>
    /// <param name="request">The provider-agnostic request payload.</param>
    /// <param name="cancellationToken">
    /// Token used to cancel the outbound call and stop enumeration (e.g. when the
    /// client disconnects mid-stream).
    /// </param>
    /// <returns>
    /// An asynchronous sequence of text chunks. Implementations should throw if the
    /// provider is not configured or the underlying request fails outright, since
    /// there is no per-chunk success/failure wrapper for streaming output; callers
    /// are expected to handle exceptions raised while enumerating.
    /// </returns>
